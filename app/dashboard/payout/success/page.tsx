...
function PayoutSuccessInner() {
  ...
  const { state, startPolling, stopPolling } = usePayout();
  ...
  const timeLabel = now.toLocaleTimeString("en-NG", {
    hour: "2-digit",
    minute: "2-digit",
  });
  ...
  return (
    <div className="mx-auto w-full max-w-2xl p-4">
      ...
      <div className="mb-6 rounded-2xl border bg-white p-6 text-center shadow-sm">
        <div
          className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full border-4"
          style={{
        ...
        {isPending ? (
          <div className="mt-3 inline-flex items-center gap-2 rounded-full bg-amber-50 px-3 py-1 text-xs text-amber-700">
            <span className="h-1.5 w-1.5 rounded-full bg-amber-600" /> Awaiting
            confirmation
          </div>
        ) : null}
      </div>
