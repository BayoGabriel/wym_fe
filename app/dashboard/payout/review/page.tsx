...
export default function PayoutReviewPage() {
  ...
  const onAuthorize = async () => {
    ...
    if ((res as any).ok) {
      setPinVisible(false);
      router.replace("/dashboard/payout/success?amount=" + encodeURIComponent(amount) +
  ...
  return (
    <div className="mx-auto w-full max-w-2xl p-4">
      ...
      <p className="mb-6 text-sm text-muted-foreground">Please confirm the details below before authorizing this transfer.</p>

      <div className="mb-4 rounded-lg border bg-white p-4">
        <div className="text-xs text-gray-500">You're sending</div>
        <div className="text-3xl font-bold">₦{Number(amount).toLocaleString("en-NG", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
        <div className="mt-2 inline-flex items-center gap-2 rounded-full bg-gray-100 px-2 py-1 text-xs text-gray-700">
          <span className="h-1.5 w-1.5 rounded-full bg-primary" /> WymLink Wallet
        </div>
      </div>
      ...
      <div className="mb-4">
        ...
        <div className="flex items-center gap-3 rounded-lg border bg-white p-3">
          ...
          <div className="flex min-w-0 flex-col">
            <span className="truncate text-sm font-medium">{accountName || "Recipient"}</span>
            <span className="truncate text-xs text-gray-500">{bankName}</span>
            <span className="truncate text-xs text-gray-500">{accountNumber}</span>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="rounded-lg border bg-white p-3">
          ...
          <div className="flex items-center justify-between py-2 text-sm">
            ...
            <span>₦{Number(amount).toLocaleString("en-NG", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
          </div>
          {description ? (
            <>
              <div className="h-px w-full bg-gray-100" />
              <div className="flex items-center justify-between py-2 text-sm">
                ...
                <span className="max-w-[55%] truncate text-right">{description}</span>
              </div>
            </>
          ) : null}
        </div>
      ...
      {state.error ? <div className="mb-3 rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-600">{state.error}</div> : null}

      <App_Button className="w-full" onClick={() => setPinVisible(true)}>Confirm transfer</App_Button>
      <p className="mt-2 text-center text-xs text-gray-500">By authorizing, you confirm that the recipient and transfer amount are correct.</p>
