...
export default function PayoutDetailsPage() {
  ...
  const { amount, description, canContinue, setAmountFormatted, setDescription, persist } = useTransferDetails();

  const onReview = (e: React.FormEvent) => {
    e.preventDefault();
    if (!canContinue || !description.trim()) return;
    const p = new URLSearchParams({ bankName, bankCode, accountNumber, accountName, amount, description });
  ...
  return (
    <div className="mx-auto w-full max-w-2xl p-4">
      ...
      <p className="mb-6 text-sm text-muted-foreground">Enter the amount and confirm the recipient details.</p>

      <RecipientSummary name={accountName} bank={bankName} accountMasked={masked} />
      ...
      <form className="mt-4 flex flex-col gap-4" onSubmit={onReview}>
        ...
        <App_Button className="w-full" disabled={!canContinue || !description.trim()}>Review transfer</App_Button>
        <p className="text-center text-xs text-gray-500">You’ll review the transfer before it is sent.</p>
      </form>
    </div>
  );
}
